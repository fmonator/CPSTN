...
void ObjectTracer::checkNewObjects(vector<FrameObject*>& objs) {
	// Vyber novy objekt ak je to potrebne (Select a new object, if necessary)
	for ( auto it = objs.begin(); it != objs.end(); ++it ) {
		FrameObject* obj = (*it);
		if(obj->hasHistory() || !canTrace(obj)) {
			continue; // trackujeme len urcite objekty (We track only certain objects)
		}
		...
		if(m_sendingPoint) {
			...
			if(result > 0.0) {
				// Pouzivatel si vybral prave tuto konturu (The user chose the contour)
				startTrace((*it));
				break;
			}
		}
...
vector<Point> ObjectTracer::mapPixels(vector<Point2f>& in, vector<Point2f>& out) {
	...
	for( i = k = 0; i < out.size(); i++ ) {
		if(!status[i]) {
			...
			// When you encounter a bug, will not receive any item in the list
		}
		out[k++] = out[i];            
	}
	out.resize(k);
...
void ObjectTracer::traceObjects(Mat& image, vector<FrameObject*>& objs) {
	for (int i=0; i < m_footprints.size(); i++) { 
		...
		// Trakuj  zvoleny objekt (Track selected object)
		if( !track->points[0].empty() ) {
			int pixelsAtInput = track->points[0].size();
			vector<Point> lostPoints = mapPixels(track->points[0], track->points[1]);

			// Ked stratim vacsinu pixelov, nerobim nic, napisem error 
			// (When lost most pixels, do not I write error)
			if(((int) (pixelsAtInput * LOST_OBJECT_PERCENT)) < lostPoints.size()) {
			...
			// (Different colors drawing points for debug view)
			Mat debugImage;
...
bool ObjectTracer::canTrace(FrameObject* obj) {
	return (
			...
			|| obj->type == PLAYER_B
			|| obj->type == REFEREE
		); // trackujeme len urcite objekty (We track only certain objects)
}
