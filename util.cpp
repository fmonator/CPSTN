...
Mat computeClusters(Mat src) {
	...
	split(src, imgRGB);
	int k = 3; // dres ma 2 casti, biela ciara, trava (jersey in 2 parts, white lines, grass)
	...
	Mat bestLabels;
	kmeans(img3xN, k, bestLabels, cv::TermCriteria(), 10, KMEANS_RANDOM_CENTERS );
	bestLabels = bestLabels.reshape(0, src.rows);
...
// (Calculate how many points there is penetration)
...
bool isRelativeIntersection(vector<Point>& contour, Rect& rec, const float& volume) {
	int similarity = intersectionPoints(contour, rec);
	return (similarity > ((int) contour.size() * volume));
}
